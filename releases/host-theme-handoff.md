# Design hand-off — the example app theme tokens

**Task:** T-063 (Sprint 07). Replace the placeholder values in `exampleAppTheme` (see `packages/core/src/theme/default.ts`) with real the example app brand tokens before staging deploy.
...
| `fontFamily` | font-family | `"Inter", system-ui, sans-serif` | Should exactly match the example app's body font stack. |
| `fontSize` | length | `14px` | Body text size. Not a heading size — tooltips are small. |

## What we do NOT need (but design might ask)

- **Dark mode.** Not v0.1. Tooltip will honour whatever colours are passed; if the example app switches themes at runtime, the host product re-calls `applyTheme(darkTheme)` and it works. We just need one theme to launch.
...
They become CSS custom properties on the host root element: `--in-app-training-primary`, `--in-app-training-background`, etc. Written by `applyTheme()` in `packages/core/src/theme/default.ts`. Any component that wants to use them just reads `var(--in-app-training-primary)`.

## Where the values go

Edit the `exampleAppTheme` object in `packages/core/src/theme/default.ts`. Then verify:
