...
### Code
- ✅ Core engine (`@in-app-training/sdk`) — Trainer, targeting, triggers, advance-on, schema, adapters, theme.
- ✅ React adapter (`@in-app-training/react`) — provider, hooks, checklist, hint, first-run.
- ✅ Vue adapter (`@in-app-training/vue`) — API-parity port of React adapter (un-deferred once we learned the example app frontend is 50/50).
...
### Content
- ✅ Sample content in `content/example-app/` — onboarding tour + create-workflow tour + hints.
...
- ✅ Rollback runbook.
- ✅ the example app data-tour PR description template.
...
- **What to do:** Take `releases/compliance-review-request.md`. Fill the `[bracketed]` sections (owner names, timelines, links). Send to whoever owns InfoSec at the org.
...
### 2. Send the `data-tour` PR to the example app frontend team  →  Estimated 1–3 business days

- **What to do:** Take `releases/adopter-data-tour-pr.md`. Fill the file paths and reviewer names. Open the PR against the example app frontend repo.
...
- **What to do:** Confirm PostHog is what the example app uses. If yes, no work. If not, swap the constructor call in `[wherever you construct the Trainer]` — one line per `docs/wiring-analytics-sink.md`.
...
- Write the actual the example app content — refine the 2 existing tours based on how they'll really flow, author the intermediate + common-task tours, refine the hints. All authorable now without env access.
- Get the real the example app brand tokens from design and swap into `packages/core/src/theme/default.ts` `exampleAppTheme` (currently placeholder values).
- Send a heads-up to the example app PM that this is coming so they can weigh in on the checklist widget's default position + label.
- Scout a second host product to be adopter #2 once v0.1 ships. The whole cross-product ROI story rests on adopter #2 showing up.
