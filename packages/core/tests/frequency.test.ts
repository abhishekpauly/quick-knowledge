...
describe('isAllowedByFrequency', () => {
  ...
  it('always allows when frequency is "always"', () => {
    expect(
      isAllowedByFrequency(
        'always',
        { status: 'completed', lastRunAt: new Date().toISOString() },
        't',
      ),
    ).toBe(true);
  });
  ...
  it('day mode: allows once per rolling 24h', () => {
    ...
    expect(
      isAllowedByFrequency('day', { status: 'completed', lastRunAt: yesterday }, 't', now),
    ).toBe(true);
    // Ran 1 hour ago — deny.
    const oneHrAgo = new Date(now.getTime() - 60 * 60 * 1000).toISOString();
    expect(
      isAllowedByFrequency('day', { status: 'completed', lastRunAt: oneHrAgo }, 't', now),
    ).toBe(false);
  });
  ...
  it('week mode: allows once per rolling 7 days', () => {
    ...
    expect(
      isAllowedByFrequency('week', { status: 'completed', lastRunAt: eightDaysAgo }, 't', now),
    ).toBe(true);
    const threeDaysAgo = new Date(now.getTime() - 3 * 24 * 60 * 60 * 1000).toISOString();
    expect(
      isAllowedByFrequency('week', { status: 'completed', lastRunAt: threeDaysAgo }, 't', now),
    ).toBe(false);
  });
});
